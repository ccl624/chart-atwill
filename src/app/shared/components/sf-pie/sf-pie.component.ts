...
export class SfPieComponent implements OnInit, AfterViewInit {
    ...
    series: [
      {
        ...
        data: [
          { name: 'bbb', value: 11, selected: true },
          { name: 'aaa', value: 11 },
          { name: 'aaa', value: 11 },
          { name: 'aaa', value: 11 },
          { name: 'aaa', value: 11 },
      ...
      },
      {
        ...
        data: [
          { name: 'aaa1', value: 11, selected: true },
          { name: 'aaa2', value: 11 },
          { name: 'aaa3', value: 11 },
          { name: 'aaa4', value: 11 },
          { name: 'aaa5', value: 11 },
  ...
  public ngAfterViewInit() {
    ...
    this.option.series.forEach((seriesItem: any) => {
      this.drawPie(seriesItem);
    });
  }
  ...
  // 添加鼠标事件
  private addMouseEvent(nodes: any, arcPathEnter: (params: any) => {}, arcPath: (params: any) => {}, centerX: number, centerY: number) {
    nodes.on('mouseenter', function (d: any) {
      d3.select(this).select('.pie-path')
        .transition()
        .duration(300)
        .attr('d', arcPathEnter(d));
      ...
      nodes.transition().duration(300).attr('transform', `translate(${centerX},${centerY})`);
  ...
  private drawPiePath(
    ...
    this.addMouseEvent(pieNodes, arcPathEnter, arcPath, centerX, centerY);
  ...
  // 画label
  private drawLabel(wrapNode: any, pieData: any[], pieParams: any, seriesItem: any, colors: string[]) {
    const centerX = pieParams.centerX;
    const centerY = pieParams.centerY;
    ...
    if (seriesItem.label.position === 'inner') {
      textNodes.append('text')
        .attr('class', 'pie-label')
        .text((d: any) => d.data.name)
        .attr('fill', (d: any, index: number) => seriesItem.label.show === false ? 'none' : '#fff')
      ...
      textNodes.append('path')
        .attr('d', (d: any) => {
          ...
          return `M${borderCenterX},${borderCenterY}
      ...
      textNodes.append('text')
        .attr('class', 'pie-label')
        .text((d: any) => d.data.name)
        .attr('fill', (d: any, index: number) => seriesItem.label.show === false ? 'none' : colors[index])
        .attr('dominant-baseline', 'middle')
        ...
        .attr('transform', (d: any) => {
          ...
          const disR = 20;
          const disX = Math.sin(d.arc) > 0 ? 1 : -1;
          return `translate(${borderCenterX + disR * Math.sin(d.arc) + disX * 25},${borderCenterY - disR * Math.cos(d.arc)})`;
        });
    }
  ...
  private drawPie(seriesItem: any) {
    const colors = seriesItem.data.map((dataItem: any, index: number) => d3.interpolateViridis(index / seriesItem.data.length));
    console.log(colors);

    const pieParams = this.getPieParams(seriesItem);
    ...
    const pieData = this.initPieData(seriesItem.data, pieParams);
    const pieGwrap = this.svg.append('g').attr('class', 'pieGwrap');
    this.drawPiePath(pieGwrap, pieData, arcPathEnter, arcPath, pieParams, colors);
    this.drawLabel(pieGwrap, pieData, pieParams, seriesItem, colors);
  }
